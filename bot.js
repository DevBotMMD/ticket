...
client.on("ready", () => {
  ...
  client.user.setGame(`Support Hevix | ${prefix}new`);
});
...
if (message.content.toLowerCase().startsWith(prefix + `new`)) {
    ...
    message.guild.createChannel(`ticket-${message.author.id}`, "text").then(c => {
        ...
        const embed = new Discord.RichEmbed()
        .setColor(0xCF40FA)
        .addField(`Hey ${message.author.username}!`, `شكرا بفتح تذكره الانتظار اسبورت حتى يشوف مشكلتك رد عليك اقرب وقت ممكن او سرع وقت ممكن`)
        .setTimestamp();
        c.send({ embed: embed });
    }).catch(console.error);
}
